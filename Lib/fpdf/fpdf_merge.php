...
declare(strict_types=1);
    ...
    class FPDF_Merge {
            ...
            switch ($item[0]) {
                ...
                case self::TYPE_DICTIONARY:
                    ...
                    foreach ($item[1] as $key => $val) {
                        if ((self::TYPE_REFERENCE == $val[0]) ||
                             (self::TYPE_ARRAY == $val[0]) ||
                             (self::TYPE_DICTIONARY == $val[0])) {
        ...
        private function _getContent($f, $xref, $content) {
            $stream = '';
            if (self::TYPE_REFERENCE === $content[0]) {
                $stream .= $this->getStream($f, $xref, $this->getObject($f, $xref, $content[1]));
            } elseif (self::TYPE_ARRAY === $content[0]) {
        ...
        private function getCompression($f, $xref, $item) {
            if (self::TYPE_TOKEN === $item[0]) {
                return [$item[1]];
            } elseif (self::TYPE_ARRAY === $item[0]) {
                $r = [];
                ...
                return $r;
            } elseif (self::TYPE_REFERENCE === $item[0]) {
        ...
        private function _toStream($item) {
            switch ($item[0]) {
                ...
                case self::TYPE_STRING: return '('.str_replace(['\\', '(', ')'], ['\\\\', '\\(', '\\)'], strval($item[1])).')';
                case self::TYPE_ARRAY:
                    $r = [];
        ...
        private function _addObj($dico = null, $buf = null) {
            $ref = $this->_newobj();
            $buf = empty($buf) && (0 !== $buf) && ('0' !== $buf) ? null : $buf;
            if (is_array($dico)) {
                if (null !== $buf) {
        ...
        public function add($filename) {
            ...
            fseek($f, (int) $startxref);
            $data = fread($f, (int) $posStartxref - (int) $startxref);
